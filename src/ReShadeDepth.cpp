...
namespace
{
	...
	void OnBeginEffects(reshade::api::effect_runtime* a_runtime, reshade::api::command_list* a_list, reshade::api::resource_view, reshade::api::resource_view)
	{
		...
		if (!g_failed && g_published && g_copyReady) {
		...
		BindDepth(a_runtime, view, updateBinding, snapshot != nullptr);
		if (output) {
			...
			if (snapshot && output->copyValue != copyValue) {
				...
				// Do not expose a previous generation while new depth is pending.
		...
		if (view.handle && !g_loggedBinding) {
			logger::info("[ReShade depth] DEPTH bound: stable output texture; completed snapshot copied on effect queue (FG depth approximation)");
...
bool ReShadeDepth::IsRequested()
{
	...
	return std::any_of(g_runtimes.begin(), g_runtimes.end(), [&desc](auto* runtime) {
		return runtime->get_hwnd() == desc.OutputWindow && runtime->get_effects_state() && g_effectDemand[runtime];
	});
}
	...
	winrt::com_ptr<ID3D12Resource>& a_retainedSnapshot, bool a_beforePresent)
{
	...
	try {
		...
		if (!g_runtime) {
			...
			logger::info("[ReShade depth] Matched game D3D12 runtime; raw render-resolution depth, no linearization");
		}
		if (!g_copyReady) {
